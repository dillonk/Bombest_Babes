...
//The slideshow starts when all the images are loaded
	...
	if(elem.length!=0){
		...
		if(navHover==true){	//if the option is true I show the next and prev button only on mouse over
			if(isMobile()){
				elem.live('vmouseover',function(){
					$('#pix_prev, #pix_next').animate({opacity:0.6},200);
				});
				elem.live('vmouseout',function(){
					$('#pix_prev, #pix_next').delay(2000).animate({opacity:0.3},200);
				});
			} else {
				elem.hover(function(){
					$('#pix_prev, #pix_next').stop(true,false).animate({opacity:0.6},200);
				},function(){
					$('#pix_prev, #pix_next').stop(true,false).animate({opacity:0.3},200);
				});
			}
		}
		...
		if(navigation==true) {	//I create the next/prev buttons
			...
			$('#pix_prev').animate({opacity:0.3},200);
		}
		...
		if(navHover==true){
			$('#pix_prev, #pix_next').animate({opacity:0.3},0);
		}
